...
private:
  ...
  void toggleGhost() { showGhost = !showGhost; }
  void toggleAnim() { doAnim = !doAnim; }
  void slam() {
    Y += getLowestY();
    placeCurTetro();
  }
  void softFall() { Y += getLowestY(); }
  ...
  void updateInfo() {
    ...
    std::string lvlStr =
        lvl > 9 ? std::to_string(lvl) : "0" + std::to_string(lvl);
    raster->addRString(12, 1 + addY, "LVL: " + lvlStr + " ",
                       lvlFlashCount < lvlFlashFrames && lvlFlashCount % 2 == 0
                           ? lvlFlashCol
                           : RasterLib::Pixel::Reset);
    ...
    raster->addRString(12, 4 + addY, scoreStr,
                       scoreFlashCount < (doAnim ? 1 : scoreFlashFrames)
                           ? scoreFlashCol
                           : RasterLib::Pixel::Reset);
  ...
  void drawBoard() {
    for (int i = 0; i < h; i++) {
      for (int j = 1; j < w + 1; j++) {
        if (board[i][j] != 0) {
          raster->setPixel(j, i + 1,
                           static_cast<RasterLib::Pixel::Color>(board[i][j]));
        }
      }
    }
  }
