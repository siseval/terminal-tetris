...
public:
  ...
  ~Raster() { delete pixels; }

  Pixel getPixel(int x, int y) { return pixels[width * y + x]; }
  Pixel getPixel(int i) { return pixels[i]; }

  void addRString(int x, int y, std::string s,
                  Pixel::Color col = Pixel::Color::Reset) {
    int cCount = 0;
  ...
  void setPixelLight(int x, int y) { pixels[width * y + x].setLight(); }
  void setPixelDark(int x, int y) { pixels[width * y + x].setDark(); }

  void setPixelSyms(int x, int y, std::string s) { getPixel(x, y).setSyms(s); }
  void setPixel(int x, int y, Pixel::Color col) {
    pixels[width * y + x].setColor(col);
  }
  void setPixel(int i, Pixel::Color col) { pixels[i].setColor(col); }
